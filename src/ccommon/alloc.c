...
void* alloc_gen_alloc(Allocator* a, void* ptr, size_t sz, int flags)
{
	if (ptr) {
		struct AllocGen * A = a->ctx;
		chunk_t *C=ptr;
		if (!chunk_used_valid_is(C)) alloc_fatal(a);
		if (sz == 0)
		{	// Free
...
/*Allocator global_allocator =
			{ &alloc_stdlib_alloc, NULL, NULL, 0 },
          *g_allocator = &global_allocator;

Allocator global_allocator_dopt =
			{ &alloc_stdlib_alloc, NULL, NULL, ALLOC_F_DOPTIMAL },
          *g_allocator_dopt = &global_allocator_dopt;*/

Allocator global_allocator =
			{ &alloc_gen_alloc, NULL, NULL, ALLOC_F_HAS_SIZE4 },
          *g_allocator = &global_allocator,
